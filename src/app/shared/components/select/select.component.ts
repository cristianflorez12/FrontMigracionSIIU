  ...
  implements ControlValueAccessor, OnInit, AfterContentInit, OnDestroy
{
  ...
  readonly selectedValue = computed(
    () => this.zValue() || this._selectedValue()
  );
  ...
  readonly selectedLabel = computed(() => {
    ...
    if (currentValue) {
      const matchingItem = this.selectItems()?.find(
        (item) => item.zValue() === currentValue
      );
      if (matchingItem) {
  ...
  private getSelectItems(): HTMLElement[] {
    ...
    return Array.from(
      dropdownElement.querySelectorAll('z-select-item, [z-select-item]')
    ).filter(
