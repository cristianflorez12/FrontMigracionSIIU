  ...
  template: `
    <div class="tab-group" [ngClass]="containerClasses()">
      ...
      <div [class]="navGridClasses()">
        @if (showArrow()) { @if (zTabsPosition() === 'top' || zTabsPosition()
        === 'bottom') {
        <button
        ...
        <nav [ngClass]="navClasses()" #tabNav>
          ...
          <button
            ...
            class="bg-gray-100 pl-8 pr-20 py-8 text-lg font-bold"
      ...
      <div class="tab-content flex-1">
        @for (tab of tabs(); track $index; let index = $index) { @if
...
export class ZardTabGroupComponent {
  ...
  public readonly zActivePosition =
  ...
  protected readonly navGridClasses = computed(() => {
    ...
    return position === 'left' || position === 'right'
      ? `grid${hasArrows ? ' grid-rows-[25px_1fr_25px]' : ''}`
      : `grid${hasArrows ? ' grid-cols-[25px_1fr_25px]' : ''}`;
  });
